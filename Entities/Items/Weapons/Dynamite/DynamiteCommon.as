...
void SetupBomb(CBlob@ this, const int fuseTicks, const f32 explRadius, const f32 explosive_damage, const f32 map_damage_radius, const f32 map_damage_ratio, const bool map_damage_raycast)
{
	...
	this.Tag("exploding");
	this.AddScript("DynamiteTimer.as");
}
...
{
	this.getSprite().PlaySound("Lighter_Use.ogg", 1.0f, 1.0f);
...
void Activate(CBlob@ this)
{
	...
	BombFuseOn(this, this.get_f32("explosive_radius"));

	this.Untag("ignore blocking actors");
	this.Untag("place45 perp");
	this.Untag("can place");
	this.Untag("cant repair");
	this.getShape().getConsts().snapToGrid = false;
}