	...
	if (
		...
		if ((blob.getNetworkID() + getGameTime()) % (moveVars.walkFactor < 1.0f ? 14 : 8) == 0)
		{
			...
			if (blob.getMap().isTileGroundStuff(tile)
                || isTileSnow(tile))
			{
				f32 pitch = 0.85f + XORRandom(150) * 0.001f;
                this.PlayRandomSound("StepSnow", volume, pitch);
			}
            else if (isTileIce(tile))
            {
                f32 pitch = 0.75f + XORRandom(150) * 0.001f;
                this.PlayRandomSound("StepIce", Maths::Min(0.2f, volume), pitch);
            }
